...
def _salva_join_data(df: pd.DataFrame, regione_input: str, output_dir: str = OUTPUT_DIR) -> str:
    """
...
def get_join_data(regione_input: str) -> pd.DataFrame:
    ...
    if os.path.isfile(path):
        ...
        df = _estrai_join_data(safe_region)
        _salva_join_data(df, safe_region, output_dir=OUTPUT_DIR)
        return df
