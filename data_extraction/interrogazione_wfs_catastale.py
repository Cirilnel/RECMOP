...
# GENERA CENTROIDI
# ========================
def _genera_centroidi_da_gdf(gdf_poligoni: gpd.GeoDataFrame, epsg_metrico: int = None) -> gpd.GeoDataFrame:
    """
...
# QUERY WFS
# ========================
def _query_catasto_point(x: float, y: float) -> dict:
    """
...
def _get_output_paths(provincia: str, comune: str) -> tuple:
    """
...
# SALVA gpkg
# ========================
def _salva_shapefile_catastale(gdf: gpd.GeoDataFrame, provincia: str, comune: str):
    """
    ...
    comune : str
        Nome del comune.
    """
    dir_path, gpkg_path = _get_output_paths(provincia, comune)
...
def _process_geodataframe(gdf_poligoni: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    cent_gdf = _genera_centroidi_da_gdf(gdf_poligoni)
    ...
    for idx, row in cent_gdf.iterrows():
        ...
        result = _query_catasto_point(x, y)
        if result:
...
def get_dati_catasto(gdf_poligoni: gpd.GeoDataFrame, provincia: str, comune: str) -> gpd.GeoDataFrame:
    ...
    if os.path.exists(gpkg_path):
        ...
        catasto_hashes = set(gdf_catasto.geometry.map(_geom_hash))
        gdf_poligoni['geom_hash'] = gdf_poligoni.geometry.map(_geom_hash)
        ...
        if not gdf_poligoni_diff.empty:
            ...
            _salva_shapefile_catastale(gdf_unito_gpkg, provincia, comune)
        else:
    ...
    gdf_unito['geom_hash'] = gdf_unito.geometry.map(_geom_hash)
    gdf_poligoni['geom_hash'] = gdf_poligoni.geometry.map(_geom_hash)
...
def refresh_dati_catasto(gdf_poligoni: gpd.GeoDataFrame, provincia: str, comune: str) -> gpd.GeoDataFrame:
    ...
    _salva_shapefile_catastale(gdf, provincia, comune)
    return gdf
