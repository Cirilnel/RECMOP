...
def _estrai_dati_siape_zc_range() -> pd.DataFrame:
    """
...
def _estrai_dati_siape_zc_suris_volris() -> pd.DataFrame:
    """
    ...
    for zona in ZONES:
        for suris_min, suris_max in SURIS_RANGES:
            for volris_min, volris_max in VOLRIS_RANGES:
                ...
                suris_label = _format_range(suris_min, suris_max)
                volris_label = _format_range(volris_min, volris_max)
...
def _estrai_dati_siape_zc_suris_volris_supdi() -> pd.DataFrame:
    """
        ...
        for suris_min, suris_max in SURIS_RANGES:
            for volris_min, volris_max in VOLRIS_RANGES:
                for supdi_min, supdi_max in SUPDI_RANGES:
                    ...
                    suris_label = _format_range(suris_min, suris_max)
                    volris_label = _format_range(volris_min, volris_max)
                    supdi_label = _format_range(supdi_min, supdi_max)
...
def _salva_dati_siape(df: pd.DataFrame, tipo: str, sep: str = ";") -> pd.DataFrame:
    """
...
def run_estrazione_siape(tipo: str) -> pd.DataFrame:
    ...
    """
    estrattori = {
        "zc_range": _estrai_dati_siape_zc_range,
        "zc_suris_volris": _estrai_dati_siape_zc_suris_volris,
        "zc_suris_volris_supdi": _estrai_dati_siape_zc_suris_volris_supdi,
    }
    if tipo not in estrattori:
        raise ValueError(f"Tipo '{tipo}' non supportato per estrazione SIAPE.")
    df = estrattori[tipo]()
    _salva_dati_siape(df, tipo)
